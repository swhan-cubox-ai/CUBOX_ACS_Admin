...
function fnIdCheck(){
	...
	if(!fnIsEmpty(fsiteid)) {
		$.ajax({
			type:"POST",
			url:"<c:url value='/admin/idDplctCnfirm.do' />",
			data:{
				"checkId": fsiteid
			},
			dataType:'json',
...
function fnSiteUserAddSave() {
	...
	if(confirm("비밀번호는 아이디 + 1234 입니다. \n저장하시겠습니까?")){
		$.ajax({
			type:"POST",
			url:"<c:url value='/admin/siteUserAddSave.do' />",
			data:{
				"fsiteid": fsiteid,
				"fname": fname,
				"fuid":fuid,
...
function fnSiteUserInfoChangeSave() {
	...
	if(confirm("저장하시겠습니까?")){
		$.ajax({
			type:"POST",
			url:"<c:url value='/admin/siteUserInfoChangeSave.do' />",
			data:{
				"fsiteid": fsiteid,
				"fname": fname,
				"fuid": fuid,
...
function fnSiteUserFuseynChangeSave(fsiteid, fuseyn) {
	...
	if(confirm(confirmTxt)){
		$.ajax({
			type:"POST",
			url:"<c:url value='/admin/siteUserFuseynChangeSave.do' />",
			data:{
				"fsiteid": fsiteid,
				"fuseyn": fuseyn2
			},
...
function fnSiteUserPasswdReset(fsiteid){
	if(confirm(fsiteid + " 비밀번호를 초기화 하시겠습니까? \n초기화시 비밀번호는 아이디 + 1234로 변경이 됩니다.")){
		$.ajax({
			type:"POST",
			url:"<c:url value='/admin/siteUserPasswdReset.do' />",
			data:{
				"fsiteid": fsiteid
			},
			dataType:'json',
...
function pageSearch(page){
	...
	f.action = "/admin/accountMngmt.do";
	f.submit();
}
