...
<script type="text/javascript">
  $(function() {
   ...
   if($("#wbTyp").val() == "BLACK"){
      $(".title_tx").html("단말기 관리 - 상세 - Black List 수정 / 등록");
    }else{
      $(".title_tx").html("단말기 관리 - 상세 - White List 수정 / 등록");
    }
  ...
  function fnInit() {
    ...
    $.ajax({
      type:"POST",
      url:"/terminal/getEmpTargetList.do",
      data:{
        "id": id,
        "wbTyp": wbTyp
      },
  ...
  function fnSearch() {
    ...
    $.ajax({
      type:"POST",
      url:"/terminal/getEmpSourceList.do",
      data:params,
      dataType:'json',
      success:function(returnData, status){
...
<form id="wbFrm" name="wbFrm">
  ...
  <input type="hidden" id="wbTyp" name="wbTyp" value='<c:out value="${wbTyp}"/>'/>
